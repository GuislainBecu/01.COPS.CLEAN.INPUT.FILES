...
source("data/read_cops.R")

ui <- fluidPage(theme = "bootstrap.css",
                shinyjs::useShinyjs(),
                titlePanel("C-OPS data cleaning ~ depth timeseries"),
                sidebarLayout(
...
server <- function(input, output, session) {
  ...
  myData <- reactive({
    if (!is.null(i.file$cpt)){
      ...
      if (any(grepl("^EuZDepth$", names(data1)))) {
        data1 <- data1 %>% 
          mutate(Depth = EuZDepth)
      } else {
        data1 <- data1 %>% 
          mutate(Depth = LuZDepth)
      }
      data1
  ...
  output$copsPlot <- renderPlot({
    ...
    if (is.null(df)) {
      ...
      dat <- df
      ggplot(dat, aes(x = date.time.num, y = Depth)) +
        geom_point() +
        scale_y_reverse() +
  ...
  output$copsPlotZoom <- renderPlot({
    df <- myData()
    if (is.null(df)) {
      dat <- data.frame(x = c(-1,0,1), y = c(-1,0,1))
      plot(0, type='n', xlab = "empty plot, select a file", ylab  = "", xlim= c(0,1), ylim = c(0,1))
      ...
      nb.toggled.points <- length(exclude$Depth)
      nb.kept.points    <- length(keep$Depth)
      nb.total.points   <- length(df$Depth)
      ...
      ggplot(df, aes(x = date.time.num, y = Depth)) +
        geom_point(data = dat.keep, aes(x = date.time.num, y = Depth)) +
        geom_point(data = dat.exclude, aes(x = date.time.num, y = Depth),
